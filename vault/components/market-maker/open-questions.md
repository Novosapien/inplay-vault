...
| T12 | **Can ONE session hold two subscriptions for the same symbol at different depths?** The MD session already holds 170 standing subscriptions at `MarketDepth=1` (MDReqID `MD-<sym>-<n>`). The full-book plan adds a second set at `MarketDepth=0` under a distinct prefix, so the top-of-book path feeding the app's price board is never touched and depth is purely additive. That needs tZERO to accept two live subscriptions per symbol on one session, distinguished only by MDReqID. If they refuse (`MDReqRejReason=1`) or silently replace the first, full book becomes a **switch-over** — subscribe everything at depth 0 and derive top of book from the best levels — which works but moves the app's live data path and needs its own verification. The 05-08 probe used `SubscriptionRequestType=0` (snapshot-only) deliberately, so it created no standing subscription and did not answer this | Full book Phase 2 | ✅ **Resolved 06-08** — tZERO ADDS rather than replaces. Both subscriptions coexisted through a live trade: depth-0 delivered New/Delete incrementals while depth-1 kept the price board current in the same window. The additive design stands and Rule 7 of the payload contract holds |
| T13 | **Is `9848` InitPrx available on the v8 feed, or IOI-only?** It carries the reference price for assets that are **not yet tradeable** — directly relevant to E3 (opening prices) and the 138 NCAA symbols sitting at TBA. `9846` is confirmed IOI-only; if `9848` is too, ask whether `MDEntryType=y` (Theoretical Opening Price, v8-only) is the equivalent. One question, not a second session | Valuation, IPO | 🔴 New _(05-08)_ |
