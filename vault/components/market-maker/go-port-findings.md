...
| **✎ 20-08 — a THIRD instance, and it changes how the file should be read** | The run-loop parity harness hardcoded `SWEEP_CADENCE_S = 2.0` from `loop.py:43`'s `# ✅ §3.1.4 — 2.0 s`. The Configuration Dictionary says **0.5**. The two engines parted at tick 1 on a single acceptor key — the sweep envelope's slot time inside its idempotency key. ⭐ **`venue`'s conclusion, worth carrying: `loop.py`'s ✅ marks are now, on the evidence, a claim to VERIFY rather than a fact to trust.** Three instances, three different numbers, one file. The permanent fix on the Go side is that both cadences now ride the manifest and are asserted, so no harness can hardcode them again |
