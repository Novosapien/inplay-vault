...
> `N41` `N74` · `inplay-sportradar-service/src/app/workers/mm_publisher/`
...
latency against a source that changes every 7 s.** Filed as `N74`, not
...
- **`N74` opened** — the publisher's 1 s tick makes every poll tier
...
3. **Then rule `N74`**, out of hours: make `tick_s` a
