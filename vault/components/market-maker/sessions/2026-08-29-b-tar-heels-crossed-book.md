...
> (the cadence check that found it) · `R-Q09` `N12` `N41` `N56` ·
...
- ✅ **George, 30-08: PROMOTE the ordering rules from `learnings.md` to
  ...
  R11 stands, and the granularity question (`N56`) is answered first.

## Questions opened / closed *(mirrored into [[market-maker/open-questions]])*

- **`N56` opened** — the guard refuses at BOOK granularity; George's
...
1. **Answer `N56`** — cost the two options (cheap: extend
...
2. Then return to the cadence work: **re-measure at peak** and rule
   `N55` (the publisher's hardcoded 1 s tick).
