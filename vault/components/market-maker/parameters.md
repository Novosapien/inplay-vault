...
| `MM identity` | `userId` / `botId` / `account` (FIX Tag 1) on the gateway | Loopback: `mm1` / `sdmm-1` / `"loopback"` · live: real user + **venue account `1797733477`** | 🟡 **rides ENV, not the dictionary** since 06-08d (`MM_USER_ID` · `MM_BOT_ID` · `MM_VENUE_ACCOUNT` via `compose.Settings` — the env-vs-dictionary split, George 06-08b) | decisions 06-08c · `mm/venue/transport.py` |
