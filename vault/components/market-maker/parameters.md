...
| `converge_max_instructions_per_tick` | ⭐ Always-quoting step 4 phase A: the most venue instructions one converger pass sends (suspends → LIVE → round-robin; books atomic) | **256** — LIVE books always fit one pass (~6 games ≈ 72 instr); the outbound twin of the drain caps | 🟡 **OURS (08-13)** — deployed in the union build · MM PR #30 | `[converge]` in `mm/venue/sync.py` |
