...
| `marketable_stall_passes` | **N41's alarm bound.** After this many CONSECUTIVE refusals of the SAME book, the guard logs `MARKETABLE_GUARD_STALLED` once per episode, naming the book, the touch holding it and the heal. A refusal keeps the target STAGED, so a book refused against a phantom touch stops publishing — this converts "an operator must notice a counter trending" into "an operator must read one alarm". An alarm, **not a mode** — it changes nothing about what is sent (sibling of `converge_staleness_alarm_s`) | **120 passes ≈ 30 s** — DERIVED: the converger runs on its own task at `converge_interval_s` **0.25 s**, and a refusal costs no budget, so a LIVE book is re-judged every pass. Long enough that a fast market crossing our staged price for a few passes stays quiet; short enough that a stuck book is heard about inside half a minute. A test pins the arithmetic, so moving either number fails the build | 🟡 ours — one-row exception approved by the lead 14-08; added + consumed in MM PR #34 (CA2 follow-up) | same |
| `boot_heal_enabled` (env `MM_BOOT_HEAL`) | F4's kill switch: the boot healer; off restores the fresh-journal-per-deploy ceremony (R-D06) | **on** | 🟡 ours — MM PR #31; healer is CA4 | same |
...
| `opening_position_shares` | R-Q08: the opening position per book until Edwin's IPO allocation publisher exists (E27) — the ask cap binds on the traded holding alone at 0 | **0 sh** | 🟡 ours / **E27** stub — MM PR #31; cap is CA3 | same |
