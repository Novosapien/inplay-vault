...
| `base_size` | 10,000 | **550** | Ask 3 — the midpoint of his "500 or 600". ⚠ Re-opens why we set 10,000: the mandate is up to 85 M shares to distribute (N20, E27). Resting size falls from 12.3% of holding to ~0.5%. ➕ **01-09: the effect is now being reported back to us as a defect.** George on the Monday touchdown: the first dry run was *"minimum three levels, maximum six levels with everything in tranches of 500... with the maximum amount of quantity being 10 to 15,000. Then I think everyone wanted it squashed right down."* The consequence, his words: *"getting a thousand filled on a market order is pretty much not going to happen unless it's coming from other people apart from the market maker."* Jared measured it independently: no order above roughly $50,000 filled in full all weekend. ⚠ **Nothing is broken. The parameter is doing what it was set to do**, and the people who asked for the cut are now describing its effect as a fault. See `N77`, `N78` and [[01-09-2026-touchdown]] | 🔴 |
...
| `levels_range` | levels per side | ✎ **1–3, drawn per dwell — DEPLOYED 27-08 (`CFG-0045`)**. Was 3–6 proposed; Edwin cut it to **1 fixed** on 20-08 (E51 answer 2, "do not build the optionality into v1"); George restored the drawn range on 27-08 after the one-rung book proved it cannot heal a bitten rung. Live shape: 86 books at 1 rung, 92 at 2, 97 at 3. ➕ **01-09: Troy wants three rungs ALWAYS, not drawn.** *"We need to smooth it out so that there's a consistent three levels."* ⚠ That is a different requirement from the drawn 1 to 3 range: it removes the draw, not just the floor. Opened as `N77` | ✅ deployed 🟡 **Edwin not yet told — book-visible** | decisions 2026-08-27 · sessions/2026-08-27-ncaa-only-maker-and-three-rungs |
