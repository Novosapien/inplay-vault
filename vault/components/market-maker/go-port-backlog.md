...
| PR #61 (`fix/malformed-replace-ack-drains`, unmerged) | 07-09 | **A replace ack that names no order drains instead of raising.** The 06-09 crash: `ORDER_REPLACED` without `orig_client_order_id`, read unconditionally ahead of the alien check → `KeyError` → exit 1 → auto-restart into a 13 GB replay. Now: `malformed_acks` counter, loud on the first and every hundredth, skip; a well-formed alien replace still drains as an alien (pinned) | **MUST reproduce** — Go's `_apply_replace` equivalent must tolerate a missing leg id the same way; check the Go tracker's replace path does not index on a missing key | 🔴 owed |
