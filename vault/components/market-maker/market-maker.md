---
description: "Component hub for the internal market maker and SNT-1 — system map, the systems status table, working-doc index and ownership boundaries"
---
...
> **Status (05-09): back up after the 1 Sep power-down — maker `supervised49`/**CFG-0047** quoting 138 NCAA books since 00:49Z; the NFL offering (32 × 900,000, GTD to 6 Sep 22:00 ET) rests beside it under bot `ipo-nfl`; the taker is OFF.** Latest: [[market-maker/sessions/2026-09-05-b-auburn-baylor-dark-window]] — AUBT + BAYB dark 9 min at their game's end (the publisher's re-offer guard held while SR sat on the final; auto-recovered; policy call **N80**) · prior [[market-maker/sessions/2026-09-05-power-up-and-nfl-offering]].
> _Prior status (14-08):_ the machine RUNS IN PRODUCTION — maker + taker, all 180 books, bus-fed, through the first live games.** Engine `supervised28`/**CFG-0026** since 11:51Z 14-08 (the bundled deploy: gateway tag-9383 forwarding `main@124991e` · the converger on its own task); taker **SNT-CFG-0019** since 12:10Z (`step4b-wash` @ `5b10d68` — the wash guard AND the boot rebase together), filling, no halts. **13-08 was the first live-game night**: the full chain ran (SR → publisher → bus → engine reprice → venue → taker fills) — after the publisher's universe filter was hotfixed 40 min before kickoff (it had NEVER adopted a game; PR #37 open, N39). The night also produced the **dead-man fire loop** (~130 sweeps as live-load ticks starved the 4 s beat window) → the window is **10 s** (env) since 00:19Z 14-08, and the **dual-engine incident** → the **single-engine lock** on the VM. **Always-quoting steps 1–4 are built AND deployed** (bounded drains · N31 group commit · progress-aware heartbeat · converger phases A+B); step 5 (the dead-man breaker) remains. ⭐ **29-08, first NCAA Saturday, measured read-only:** the engine holds its cadence — tick and sweep 0.5 s, **`missed_intervals=0` across 1,423 sweeps**, CPU **7.5% of one core** — and **Sportradar, not the machine, is the limit**: an NCAA probability changes every **7 s** at the median (p90 25 s). ⚠ Measured at ONE to SEVEN live games, with 65 of 83 tracked games still pre-kickoff — **not the peak**. Two faults found: `IPTCNCTH` frozen behind the marketable guard (`R-Q09`/`N41`), and the publisher's hardcoded 1 s tick making `MMPUB_POLL_LIVE_S=0.5` unreachable (**N74**; real poll 1.16 s, cost ~0.3 s). ⭐⭐ **29-08b — Edwin's "asks and no bids" traced:** `IPTCNCTH` (and `IPTCHFRG`) sat **SELF-CROSSED for 2 h 45 min** during a live game — the R-Q09 guard refuses a WHOLE BOOK, so both sides froze; it recovered alone when the touch moved. ⭐ **We designed the fix on 23-07 and never shipped it** — George's ordering rules (cancels before creates · **micro-barrier on the specific crossing orders**) sat unpromoted in [[market-maker/learnings]], so flat post-first shipped and R-L03 codified it. **Promoted to a decision 30-08**; granularity is **N75**. See [[market-maker/sessions/2026-08-29-b-tar-heels-crossed-book]]. See [[market-maker/sessions/2026-08-29-ncaa-saturday-cadence-check]]. Standing faults queued in [[market-maker/build-deploy-log]]: ~35% missed sweeps under live load (measure → design fixes → speed work; ✎ did not reproduce 29-08 at low game count), the fresh-journal `p_ref` rebase erasing live moves on cutover (BENG/LION, N22 violated), and **N40** — game-end feed retirement suspends books with no re-open path (service fix built + on the testing pool, PR #38; prod pending; engine-side hand-off open). ⚠ **Check [[market-maker/build-deploy-log]] before deploying anything** — several sessions operate this machine in parallel. Earlier arc: chapters built 24-07 → 06-08 (event core → valuation → position → quoting → market state → venue → runtime, replay byte-identical, §10.3 checkpoints equality-proven); first real-venue day 07-08; full-book 180 run 08-11; state publishers + manual taker orders 12-08b. Live mode still refuses on its gates (S1 landed; T1 · N19 remain). Baseline: the **v1.3 Build Spec** (24-07), superseded where [[market-maker/decisions]] says so. Repo: `inplay-market-maker` (Python), production lineage `feat/always-quoting-step4b` · **E11 settlement + E12 NCAA still unasked**
...
| [[market-maker/systems/mm-ops-ui\|MM Ops UI]] | Admin-panel monitoring/control: positions, P&L, books, the taker's manual order ticket; algo params later | ⭐ Phase 1 discovered 12-08 (`specs/2026-08-12-admin-trading-observability/`) · **engine-side state publishers + the manual ticket BUILT 12-08b** (four review rounds, 818 tests) · **publishers DEPLOYED 08-13** (the union build; the panel renders `/mm`) · will own CONFIGURATION_ACTIVATION + the N19 upload page |
| [[market-maker/systems/snt-1-noise-taker\|SNT-1 — the Market Taker]] | Edwin's house noise taker: crosses the MM's spread with random clips so every book prints trades | ⭐ **Built 08-08 · DEPLOYED 08-11, running since** — own account `4963224393`, all 180 books, AUTO states off the bus (T-F07), wash guard, boot rebase; shorts merged, OFF · **LIVE rate ruled 15-08: one print a second (MM #40, built, not deployed — the running taker still fires every 5.3 s)** · compliance rulings still owed (E32 numbers · E33/T13 · IPLP class T-I02) · requirements: [[market-maker/market-taker-requirements]] |
| [[market-maker/systems/expected-wins-pipeline\|Expected-Wins Pipeline]] | **The pricing centre** — seed expected wins once from de-vigged win totals, then the zero-jump absorber; the journal is the store; external generator feeds the engine untouched | ⭐ **Ruled 26-08** ([[market-maker/decisions]]) — replaces the [[market-maker/systems/daily-reference-feed\|daily-file design]] (superseded: Edwin cannot operate a daily hand-off) · closes N18/N19/N23 + N22's residual · build this week, ⚠ games live Saturday 29-08 · Edwin's one-time blessing = E51 |
| [[market-maker/systems/suspension-recovery\|Suspension Recovery]] | George's requirement: every suspension needs a defined exit — the terminal-suspension alarm, the anchor seed's freshness carry, the manual release lever, the quarantine exit | ⭐ **Designed 16-08** — key finding: **input-driven auto-recovery ALREADY exists** (`restriction()` is pure, leaving Suspended is dwell-free), so the 15-08 dead end was an input that could never recover, not a latch. Four gaps specified; build gated on George's approval (+ N23/N28 for the seed extension, panel access control for the lever) |
