...
> **Status (05-08b): the machine RUNS — `python -m mm.runtime` boots, stands its book at the gateway, ticks, and restarts from its journal.** Chapters 3–8 + 12, the sweep event (N28), the clock-owning runtime, tiered polling (LIVE 2 s → OVERNIGHT 30 min), the 170-security universe from tZERO's own tickers, and the composition — **500 tests**, drill-proven on the rig 05-08 (cold boot + restart). ⭐ E38 deviation built: **a successful fetch confirms the number** — the book quotes at full status through halftime and suspends only on 20 s of true feed silence. Live mode **refuses to start** until its gates close: the sr-id bindings + the live HTTP source (⭐ **buildable NOW on the trial key** — George 05-08) · Edwin's file delivery (N19). External: **T1** (→ Hasan, with N30 + the governor) · **E27** (the day-one book) · the unsent round **E29–E38**. Still owed: §10.3 checkpoints (required — every deploy is a restart) · the boot-reconcile healer (gap demonstrated live in the restart drill, parked) · §3.6 off-field · Ch 9 IPO · Ch 11 settlement. Baseline: the **v1.3 Build Spec** (24-07), superseded where [[market-maker/decisions]] says so. Repo: `inplay-market-maker` (Python), branch `feat/position-engine`
