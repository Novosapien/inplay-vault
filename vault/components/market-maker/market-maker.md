...
> **Status (05-08): the RUNTIME is built and the machine survives real feed rhythm.** Chapters 3–8 + 12, the §3.1.4 sweep (N28, the tenth event type) and `mm/runtime/` (the one clock-owning loop) built — **474 tests**, wire-proven 02-08. ⭐ New deviation, E38: **a successful fetch confirms the number** — the live staleness bands run on observation age, so the book quotes at full status through halftime (a measured 2,862 s feed gap) and suspends only on 20 s of true silence. Deployment designed 03-08 (N7: own VM + the trading admin panel, joined by NATS). Remaining blockers are permissions and answers, not code: **T1** (MM account + ACL) · **S1/S7** (SR entitlement — the live HTTP source and the liveness producer both gate on it) · **E27** (the opening position) · the unsent Edwin round **E29–E38**. Season-scale build still owed: tiered polling · the composition script · §10.3 checkpoints (required — every deploy is a restart) · §3.6 off-field · Ch 9 IPO · Ch 11 settlement. Baseline: the **v1.3 Build Spec** (24-07), superseded where [[market-maker/decisions]] says so. Repo: `inplay-market-maker` (Python), branch `feat/position-engine`
