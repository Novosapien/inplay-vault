...
| **[[customer-onboarding/customer-onboarding\|Customer Onboarding]]** | Discovery → install → registration+KYC → wallet provisioning → trading | 5 sub-components: Discovery & App Acquisition, Registration+KYC, Wallet Provisioning (tZERO, pre-funded pool proposed), Holding State (gray out, never hide), Returning Login (tZERO auth + device biometric). No SSO at launch. Email-only manual field. | **Defined** |
| **[[information-layer/information-layer\|Information / Bloomberg Terminal]]** | The data and intelligence layer -- the main stage of the app. Covers "Discover -> See -> Understand -> Decide" user journey | Six sub-components: Discovery/Home, Game Day Overview, Single Game Page, Team Page, Research Tab (**Defined 26-06**, 3-tier paid build: pre-canned → saved-custom → AI companion; 99c→$14.99→$49.99 tiers, in-app payment; **pricing 22-07/24-07: four-tier ladder proposed (Free trial / Plus $24.99 / Pro $49.99 / Elite $79.99), tier prices under review, not a launch feature (target ~October); pre-canned report catalog supplied 24-07**), Leaderboard. Consumes SR (sports data, match tracker, news) + tZERO (prices, order book). Owns the cross-correlated volatility dataset (SR events x tZERO prices). News feed, block trade alerts, leaderboards (3 verticals x 4 time horizons), proximity alerts | Defined |
| **[[ipo-module/ipo-module\|IPO Module]]** | The "Trading Challenge Draft" — how every tradeable asset is issued before secondary trading opens, plus season-end liquidation | 6 sub-components: Draft Board (Tinder/list/filter browse), Team IPO Detail, Primary Offering Execution (5M float, static ask, buy-only, no cap, 20% short holdback), Scheduling & Windows (72h, NCAA ~Aug 20 / NFL ~7d pre-Sept 9), Announcement & Countdown, Season-End Settlement. Issuer = team treasury (on-chain via tZERO). Seeds the off-field value mechanic that feeds Earnings Report | Defined |
...
| **[[market-maker/market-maker\|Market Maker]]** _(new, promoted 20-07-2026)_ | **Internal, not user-facing.** Synthetic market-maker entity in tZERO, posts two-sided resting liquidity in every team market, guarantees IPO fill (float warehousing), maintains orderly markets (price band + quote busting), and generates the behavioural dataset for production MMs. **Now also houses a second house agent, SNT-1 (Synthetic Noise Taker, Edwin 30-07):** a taker-only, non-participant account that crosses the spread with random noise so every book trades from IPO onward (see [[market-maker/systems/synthetic-noise-taker]]) | Unlimited buying power + short-locate exemption; market-state-driven quoting (reference price ± offsets, skew, randomizer); limit-orders-only with crossing; cancel-replace ~5–10×/sec; three liquidity sessions; CTS1/CTS2 price engines built by InPlay; ops via desktop app version (Kevin). Deep-dive Thu 23-07 | **Collecting** |
| **Withdrawal Flow** _(new — surfaced 12-06-2026)_ | Conversion from InPlay$ to real cash — bank info capture, crypto wallet linking, 1099, eligibility verdict surfacing | Captured at first withdrawal request (not signup). Crypto wallet option via Coinbase (Iris conversation referenced). tZERO cash wallet hosting. Receives eligibility verdict from Referral. Will be large. **24-07: payouts named the launch blind spot** — payment-provider deal unsigned/unintegrated; accepted worst case = users see amounts owed, actual payout delayed a couple of weeks; Edwin fine with an interim manual rail (Zelle/wire — "the payment processor is starting to become a bugaboo of mine"). Tax forms also in the launch remainder. (Source: standup 2026-07-24) | Stub |
...
- **Advertising / Ad Serving:**
  ...
  - **Education-on-tokenization angle (Edwin):** tZERO partnership creates a valuable educational ad inventory around tokenization/blockchain literacy
  ...
  - **First SSP going live (24-07):** the Apple app store ID landed 23-07, unblocking ad-network verification — **AdMob verification in progress (24–48h)**, giving the first serving SSP so datasets start flowing. Android follows the same process once its store listing is live; the remaining SSPs all need app store IDs/URLs and run on their own timelines. (Source: standup 2026-07-24)
...
- **Analytics & Funnel Measurement** _(new — surfaced 12-06-2026):_
  ...
  - **Google Tag Manager going in (24-07):** Hasan installing GTM — one container installed once, then any tag drops in without releases (Google Analytics, HubSpot, MMP, Facebook, cookies). Cody to disseminate the setup to marketing/content partners for cross-platform content tagging. (Source: standup 2026-07-24)
  - **MMP evaluation open (24-07):** **Kochava vs AppsFlyer** — prospective agency **Plexus** already runs Kochava, so Cody suggested consolidating on it for simplicity; Brett reviewing and owes honest feedback. No decision; Plexus proposal not yet in. (Source: standup 2026-07-24)
...
- **Cybersecurity & Data-Handling Framework** _(new — surfaced 12-06-2026):_
  ...
  - Specifically applies to: PII from Persona, biometric data, location data (Referral), bank info (Withdrawal Flow), wallet ledgers (tZERO)
...
- **Data Replication & Derivative-Data Products** _(new, surfaced 22-07-2026):_
  - Plan to **replicate everything tZERO stores** as a backup and to build **real-time derivative data products** on top (InPlay cannot depend on tZERO latency for live derived data)
  ...
  - No clean home today; candidate for a dedicated architecture concern doc. Touches tZERO integration, Market Data Service, Research Tab (derivative products). See [[open-questions]]
