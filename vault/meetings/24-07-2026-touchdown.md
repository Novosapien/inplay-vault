...
| **Trading infra mapped 23-07;** launch-readiness **non-negotiable**. Remaining items small (**cancel feature**, **cancel-and-request**), tightly integrated with MM; some waited on tZERO. **Testing via historical-game simulation** (Chiefs vs Ravens), run multiple times/day, on a test app version. | [[trading/trading]] | 24-07 update block |
...
| **tZERO call cadence changed:** tZERO tech calls no longer Fridays, now **Tue/Thu**; Novo touchdowns **Mon/Wed/Fri**. | Meeting doc only | Status |
...
**Source transcript:** `Inplay - App - Touchdown – 2026_07_24 14_34 BST – Notes by Gemini.md` (raw, in `/home/brett/shared/inplay/meeting-notes/`). The digest above is paired with the full transcript below (carried in from the MM-branch capture of the same call).


### MM-side extraction record (processed separately on the MM branch, same day)
