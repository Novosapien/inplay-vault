...
| **Data replication + derivative data products:** replicate everything tZERO stores as a backup on InPlay's systems, plus real-time derivative data products (can't rely on tZERO latency); **cold storage after N days** (a fraction of warm cost); **~5–6yr** regulatory retention (Edwin first said 10). Brett flagged compute/storage costs become real numbers, some of it is dead / non-monetizable. | Meeting doc only (candidate, see below) | Flagged as a NEW component/concern candidate for the orchestrator |
