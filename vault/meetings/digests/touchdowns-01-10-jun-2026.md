...
- **Wallet ownership clarified (10-06):** the **referral wallet is tracked by InPlay** (not tZERO); a **mechanism is needed to move funds from the referral wallet back into the trading wallet** (the <25K reload). _(10-06 — see Trading)_
...
- **Press release:** Skye drafted a **tZERO press release**; tZERO is also drafting one → **merge and time for Tuesday**. _(05-06)_

### Trading
- **tZERO wallet / buying-power ownership RESOLVED (10-06):** **tZERO owns and manages the trading wallet** (tied to the digital wallet). **InPlay tracks the referral wallet** and must build the **<25K reload mechanism** (referral → trading). **Cash wallet host = still TBD** (tZERO vs third party). **Buying power: tZERO does *not* calculate it** — they only do that in production *as the broker*. **InPlay must build an "InPlay market synthetic broker" element that tracks buying power.** ⚠️ Business requirements not yet written — Friday tZERO session. _(10-06)_
...
- *(May/early-June architecture signal — VPC + tZERO integration — captured in the Trading doc's architecture note.)*
...
| **InPlay synthetic broker (buying-power tracker)** | 10-06 | New architectural element — InPlay (not tZERO) must track buying power, as the synthetic broker; tZERO only does this in production as the real broker | 🚩 **Open question RESOLVED + Trading note added; flagged for the Friday focused session** (business requirements being written). Not a standalone component doc yet |
...
4. ✅ **InPlay Global Website** — update block + Page Map: redesign on deck copy + brand colours, **imagery-last**, **outline font fixed**, **Partners/Team/Football-Challenge/Careers** pages, **CTA→form reason-routing to DLs**, publish-before-conference, tZERO press-release merge, **mobile-optimisation rework**, **compliance-incident note** (guaranteed→up-to + pre-deploy copy review + counsel disclaimer review).
5. ✅ **Trading** — architecture note: **VPC up + FIX gateway rebuilt + tZERO integration working**; **wallet/buying-power split RESOLVED** (tZERO = trading wallet; InPlay = referral tracker + **synthetic broker** for buying power; cash wallet TBD; reload mechanism); **shorting mechanics**; synthetic broker + shorting BRs flagged for Friday.
