...
| **Subscription management** | Full order book depth for all watchlist symbols wastes bandwidth. | Full depth only for the actively viewed symbol. Watchlist gets top-of-book only (MarketDepth=1). Unsubscribe when user navigates away. Matches tZERO spec lazy subscription pattern. |
